...
//      fold-in size (~1.7 MB pre-split; post-split it was ~520 kB and is
//      ~670 kB as of the 2026-09 dependency bumps, so 800 kB still catches
//      a vendor fold-in while leaving headroom for app growth).
//
...
/** Bytes. Baseline ~670 kB (2026-09; ~486 kB at #307); fold-in exceeds ~1.6 MB. */
		...
		if (size > INDEX_CHUNK_CEILING) {
			failures.push(
				`Entry chunk ${indexChunk} is ${(size / 1024).toFixed(0)} kB, over ` +
					...
					`dependency has likely folded back into the app chunk (baseline: ` +
					`~670 kB as of 2026-09). Check the advancedChunks groups in ` +
					`vite.config.ts.`,
			);
		}
	}
