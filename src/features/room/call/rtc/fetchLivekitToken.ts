 ...
 * convention exposes the JWT issuer at `${base}/livekit/sfu/get`; older
 * tooling sometimes stores just the base. Phase 1's `discoverFoci` always
 * writes the full path, but we accept either shape so `.well-known`-sourced
 * foci (Phase 3+) keep working.
 */
function normaliseJwtServiceUrl(input: string): string {
	const trimmed = input.trim().replace(/\/+$/, "");
	if (trimmed.endsWith("/sfu/get")) return trimmed;
