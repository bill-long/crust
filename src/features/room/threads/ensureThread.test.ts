...
}): Thread & { __emitUpdate: () => void; __setFetched: () => void } {
	const listeners = new Set<(...args: unknown[]) => void>();
	const thread = {
		id: "$root",
		initialEventsFetched: overrides?.initialEventsFetched ?? true,
		on: (event: unknown, handler: (...args: unknown[]) => void) => {
			if (event === ThreadEvent.Update) listeners.add(handler);
			return thread;
		},
		off: (event: unknown, handler: (...args: unknown[]) => void) => {
			if (event === ThreadEvent.Update) listeners.delete(handler);
			return thread;
		},
		__emitUpdate: () => {
			for (const h of [...listeners]) h(thread);
		},
		__setFetched: () => {
