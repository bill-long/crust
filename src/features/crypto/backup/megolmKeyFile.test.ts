...
describe("megolmKeyFile", () => {
	...
	it("produces the documented binary structure (version ‖ salt ‖ iv ‖ iters ‖ ct ‖ hmac)", async () => {
		...
		const body = requiredAt(file.split("\n"), 1, "export body");
		...
		expect(requiredAt(packed, 0, "format version")).toBe(1);
	...
	it("rejects a tampered file (HMAC mismatch)", async () => {
		...
		const body = atob(requiredAt(lines, 1, "export body"));
	...
	it("rejects truncated files", async () => {
		const file = await encryptMegolmKeyFile(SAMPLE_JSON, "pw", ITERS);
		const lines = file.split("\n");
		const short = `${requiredAt(lines, 0, "export header")}\n${requiredAt(
			lines,
			1,
			"export body",
		).slice(0, 20)}\n${requiredAt(lines, 2, "export footer")}`;
	...
	async function mutatePacked(
		...
		const packed = Uint8Array.from(
			atob(requiredAt(lines, 1, "export body")),
			(c) => c.charCodeAt(0),
		);
		mutate(packed);
		let binary = "";
	...
	it("tolerates whitespace-wrapped base64 bodies", async () => {
		const file = await encryptMegolmKeyFile(SAMPLE_JSON, "pw", ITERS);
		const lines = file.split("\n");
		const wrapped = requiredAt(lines, 1, "export body").replace(
			/(.{40})/g,
			"$1\n",
		);
		const file2 = `${lines[0]}\n${wrapped}\n${lines[2]}`;
